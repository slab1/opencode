...
KANBAN_STATUSES = ["draft", "scheduled", "posted", "analyzed", "archived"]
...
def add_post(text: str, platforms: list, schedule: str, media: str = "",
             ...
             recurring: str = None, title: str = "",
             initial_status: str = None) -> str:
    ...
    if schedule.lower() in ("now", "asap"):
        ...
        try:
            ...
            when = dt.isoformat()
            status = initial_status or "scheduled"
        except ValueError:
...
def set_status(post_id: str, new_status: str) -> None:
    """Change the status of a calendar post."""
    if new_status not in KANBAN_STATUSES:
        print(f"{C.R}Error: Invalid status '{new_status}'. Choose from: {', '.join(KANBAN_STATUSES)}{C.N}")
        sys.exit(1)
    ...
    for p in cal["posts"]:
        if p["id"] == post_id:
            ...
            save_calendar(cal)
            with LOG_FILE.open("a") as f:
                f.write(f"[{datetime.now().isoformat()}] STATUS: {post_id} {old_status} → {new_status}\n")
            print(f"{C.G}✓{C.N} {post_id}: {old_status} → {new_status}")
            return
    print(f"{C.R}✗{C.N} Not found: {post_id}")


def kanban_board() -> None:
    """Show posts grouped by kanban status column."""
    cal = load_calendar()

    print(f"\n{C.CY}{'═' * 75}{C.N}")
    print(f"{C.CY}  Kanban Board — Content Pipeline{C.N}")
    print(f"{C.CY}{'═' * 75}{C.N}")

    # Kanban column display config
    columns = {
        "draft":     (C.Y, "📝 Draft"),
        "scheduled": (C.B, "📅 Scheduled"),
        "posted":    (C.G, "✅ Posted"),
        "analyzed":  (C.M, "📊 Analyzed"),
        "archived":  ("\033[90m", "📦 Archived"),
    }

    for status in KANBAN_STATUSES:
        posts = [p for p in cal["posts"] if p["status"] == status]
        color, label = columns.get(status, (C.N, status))
        print(f"\n{color}━━━ {label} ({len(posts)}){C.N}")

        if not posts:
            print(f"  {color}(empty){C.N}")
            continue

        # Show at most 8 per column to keep it scannable
        for p in posts[:8]:
            when = datetime.fromisoformat(p["schedule"])
            platforms = ", ".join(p["platforms"][:2])
            if len(p["platforms"]) > 2:
                platforms += f" +{len(p['platforms']) - 2}"
            preview = p["text"][:50] + "..." if len(p["text"]) > 50 else p["text"]
            print(f"  {color}•{C.N} {p['id'][:30]:30} {when.strftime('%m/%d %H:%M'):14} {platforms:20}")
            print(f"    {preview}")

        if len(posts) > 8:
            print(f"  {color}... and {len(posts) - 8} more{C.N}")

    # Summary bar
    total = len(cal["posts"])
    counts = "  ".join(f"{C.Y}{len([p for p in cal['posts'] if p['status'] == s])}{C.N} {s}" for s in KANBAN_STATUSES)
    print(f"\n{C.CY}{'─' * 75}{C.N}")
...
def main():
    ...
    # add
    add_p = sub.add_parser("add", help="Add a post to the calendar")
    ...
    add_p.add_argument("--status", default=None, choices=KANBAN_STATUSES,
                       help="Initial kanban status (default: 'draft' if future, 'pending' if now)")
    ...
    # kanban board
    sub.add_parser("kanban", help="Show kanban board grouped by status")

    # status (change status)
    status_p = sub.add_parser("status", help="Change a post's kanban status")
    status_p.add_argument("post_id", help="Post ID to update")
    status_p.add_argument("--set", dest="new_status", required=True,
                          choices=KANBAN_STATUSES, help="New kanban status")
    ...
    if args.cmd == "add":
        ...
        set_status(args.post_id, args.new_status)
