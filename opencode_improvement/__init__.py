...
class MockProvider:
    ...
    def evaluate(self, agent_name: str = None) -> List[dict]:
        ...
        results = []
        for tc in cases:
            # Mock provider always passes: simulates perfect agent behavior
            results.append({
                "id": tc.get("id", ""),
...
# ═══════════════════════════════════════════════════════════════════════════
# Improvement #10: Task Versioning
# ═══════════════════════════════════════════════════════════════════════════


def list_task_versions() -> dict:
...
def get_baseline_version_info() -> dict:
...
# ═══════════════════════════════════════════════════════════════════════════
# Improvement #6: Cohen's Kappa — inter-rater agreement
# ═══════════════════════════════════════════════════════════════════════════
...
def _cohens_kappa(ratings_a: List[int], ratings_b: List[int]) -> float:
    ...
    Args:
        ratings_a: Binary ratings from rater A (0 or 1).
        ratings_b: Binary ratings from rater B (0 or 1).

    Returns:
        Kappa value between -1 and 1. < 0.7 indicates poor agreement.
    """
...
def compute_cohens_kappa(dataset_path: str = None) -> dict:
    """Compute Cohen's Kappa for the golden dataset by comparing
    two simulated rater judgments (keyword-based vs property-based).

    Args:
        dataset_path: Path to golden dataset JSON.

    Returns:
        dict with kappa scores per category and overall.
    """
    ...
    # Rater A: keyword-based (checks if reference keywords appear in input)
    # Rater B: property-based (checks if expected behavior matches structural audit)
    ratings_a = []
    ratings_b = []
...
# ═══════════════════════════════════════════════════════════════════════════
# Improvement #1: LLM-as-Judge Evaluator
# ═══════════════════════════════════════════════════════════════════════════
...
class LLMJudge:
    ...
    def score(self, test_case: dict, agent_output: str = None) -> dict:
        ...
        Uses LLM if endpoint configured, otherwise uses heuristic fallback.

        Args:
            test_case: The golden test case dict.
            agent_output: The agent's actual output text (optional).

        Returns:
            dict with per-metric scores (0.0-1.0) and overall.
        """
        if self.available:
    ...
    def _heuristic_score(self, test_case: dict, agent_output: str) -> dict:
        """Heuristic fallback scoring without LLM endpoint."""
        desc = (test_case.get("description", "") + " " + test_case.get("reference", "")).lower()
        output = (agent_output or desc).lower()

        # Faithfulness: does the agent mention key concepts from the task?
        ...
        # Task completion: does the output match expected behavior?
        ...
        # Answer relevancy: how many reference keywords appear?
    ...
    def _llm_score(self, test_case: dict, agent_output: str) -> dict:
        ...
        When an LLM endpoint is configured, this sends the test case
        and agent output for evaluation. For now, falls back to heuristic.
        """
        # TODO: Implement actual LLM API call when endpoint is configured
        return self._heuristic_score(test_case, agent_output)

    def evaluate_batch(self, golden_results: List[dict]) -> List[dict]:
        """Add LLM-judge scores to a batch of golden results."""
        enriched = []
...
# ═══════════════════════════════════════════════════════════════════════════
# Improvement #9: Multi-Executor (SyncExecutor / AsyncExecutor)
# ═══════════════════════════════════════════════════════════════════════════
...
class SyncExecutor:
    ...
    def execute(self, func, items: List[Any], *args, **kwargs) -> List[Any]:
        """Run func on each item sequentially."""
        results = []
        for item in items:
...
class AsyncExecutor:
    ...
    def execute(self, func, items: List[Any], *args, **kwargs) -> List[Any]:
        """Run func on each item concurrently using asyncio.gather."""
        try:
            import asyncio
...
# ═══════════════════════════════════════════════════════════════════════════
# Improvement #7: Tiered Scorecard (ASCII bar charts)
# ═══════════════════════════════════════════════════════════════════════════
...
def render_scorecard(eval_result: dict) -> str:
    ...
    Args:
        eval_result: Result dict from eval_agents().

    Returns:
        Formatted string with ASCII bar charts per agent.
    """
    lines = []
    lines.append("=" * 60)
    ...
    lines.append("")
    lines.append("  Legend: █ = pass  | color: GREEN >= 0.8, YELLOW >= 0.6, RED < 0.6")
    lines.append("=" * 60)
...
# ═══════════════════════════════════════════════════════════════════════════
# Improvement #8: Per-Case Inspect
# ═══════════════════════════════════════════════════════════════════════════


def inspect_case(agent_name: str, case_id: str = None, failed_only: bool = False) -> dict:
    """Inspect a golden test case or list failed cases for an agent.

    Args:
        agent_name: Agent to inspect.
        case_id: Specific test case ID (e.g., "web-browser-001").
        failed_only: If True, show all failing test cases for the agent.

    Returns:
        dict with full test case details.
    """
    if not GOLDEN_FILE.exists():
        return {"status": "error", "message": "Golden file not found"}
    ...
    # Run eval to get pass/fail status
    try:
    ...
    if case_id:
        ...
        return {"status": "error", "message": f"Case '{case_id}' not found for agent '{agent_name}'"}

    if failed_only:
        # Show all failing cases
        failing = []
        for case in agent_cases:
            cid = case.get("id")
...
# ═══════════════════════════════════════════════════════════════════════════
# Improvement #3: Comparison Report (markdown)
# ═══════════════════════════════════════════════════════════════════════════


def generate_comparison_report(eval_result: dict, baseline_path: str = None) -> dict:
    ...
    for agent_name, stats in sorted(current_agents.items()):
        ...
        # Look up baseline (from audit data)
        baseline_agent = baseline_agents.get(agent_name, {})
        baseline_pr = 1.0  # Default if not in baseline
    ...
    # Also save JSON for trend tracking
    trend_json = {
...
# ═══════════════════════════════════════════════════════════════════════════
# Improvement #5: A/B Agent Config Comparison
# ═══════════════════════════════════════════════════════════════════════════
...
def ab_compare_agents(
    ...
    Args:
        ...
        config_a: Path to incumbent agent config (.md).
        config_b: Path to candidate agent config (.md).
    ...
    def score_config(config_path: str) -> dict:
        """Score a single agent config against golden test cases."""
    ...
    # Per-metric comparison (candidate must beat incumbent on ALL metrics)
    all_beat = True
    per_case = []
...
# ═══════════════════════════════════════════════════════════════════════════
# Agent Evaluation (DeepEval + Juanllenato fail-under + Victor-David-Medina baseline)
# ═══════════════════════════════════════════════════════════════════════════
...
def eval_agents(
    ...
    - Mock provider for offline eval
    - LLM-as-judge scoring
    - Multi-executor (sync/async)
    - Tiered scorecard output
    - Version-aware evaluation

    Args:
        agent_name: Specific agent to evaluate, or None for all
        config_path: Path to YAML eval config file (optional)
        use_golden: Whether to run golden test cases (default: True)
        fail_under: Minimum pass rate (0.0-1.0) — exit non-zero if below
        severity: Minimum severity level to fail on (info/warn/critical)
        compare: Whether to compare against baseline
        baseline_path: Path to baseline JSON for regression detection
        provider: "real" or "mock" — mock uses MockProvider for offline eval
        judge_model: LLM model name for LLM-as-judge scoring (None = use heuristic)
        executor_type: "sync" or "async" — execution strategy
        scorecard: Whether to render ASCII scorecard

    Returns:
        dict with eval results including per-agent scores, pass rate, and regressions
    """
    # 0. Use mock provider if requested
    if provider == "mock":
        mock = MockProvider.from_file(str(GOLDEN_FILE) if use_golden else None)
        ...
        if compare or baseline_path:
            bp = baseline_path or (EVAL_DIR / "baseline.json")
        ...
        # Apply LLM judge scores if requested
        if judge_model:
            judge = LLMJudge(model=judge_model)
            result["golden_results"] = judge.evaluate_batch(result["golden_results"])

        # Apply scorecard if requested
        if scorecard:
            result["scorecard"] = render_scorecard(result)

        # Generate comparison report if requested
        if compare:
            report = generate_comparison_report(result, baseline_path)
    ...
    # 2. Run golden test cases if requested
    golden_results = []
    if use_golden and GOLDEN_FILE.exists():
        golden = json.loads(GOLDEN_FILE.read_text())
        ...
        def evaluate_single_tc(tc):
            ...
            if agent == "all":
                # Property-based tests: run against ALL agents using audit data
                ...
                # Behavioral tests: keyword match against agent config
    ...
    # 5. Apply LLM judge scores if requested
    if judge_model:
        judge = LLMJudge(model=judge_model)
        golden_results = judge.evaluate_batch(golden_results)

    # 6. Baseline comparison (Victor-David-Medina pattern)
    regression = None
    if compare or baseline_path:
        bp = baseline_path or (EVAL_DIR / "baseline.json")
        if bp and Path(bp).exists():
    ...
    # 7. Apply scorecard if requested
    if scorecard:
        result["scorecard"] = render_scorecard(result)

    # 8. Generate comparison report if requested
    if compare:
        report = generate_comparison_report(result, baseline_path)
        result["comparison_report"] = report
...
# ═══════════════════════════════════════════════════════════════════════════
# Metacognitive Strategy Tracking (HyperAgents-inspired)
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# Strategy Library — all known improvement strategies (HyperAgents pattern)
# ═══════════════════════════════════════════════════════════════════════════
...
def audit_agents(agent_name: str = None) -> dict:
    """Audit all agent configs for structural completeness.

    Args:
        agent_name: Specific agent to audit, or None for all.

    Returns:
        dict with audit results per agent.
    """
    if not AGENTS_DIR.exists():
        return {"status": "error", "message": f"Agents dir not found: {AGENTS_DIR}", "agents": []}
    ...
    for fpath in files:
        ...
        if text.startswith("---"):
            ...
            if end > 0:
                ...
                agent_info["frontmatter_has_permission"] = "permission:" in fm or "task: allow" in fm or "task: allow" in text
...
def generate_report(agent_name: str = None) -> dict:
...
def suggest_improvements(agent_name: str) -> dict:
