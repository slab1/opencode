...
PLUGIN_COUNT=$(ls "$CONFIG_DIR/node_modules/"@* 2>/dev/null | wc -l)
...
overdue = 0
commitments = data if isinstance(data, list) else data.get('commitments', [])
for c in commitments:
    if c.get('status') == 'open' and c.get('due_at'):
        try:
