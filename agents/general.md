...
1. **Task Analysis**: Decompose the task into research, analysis, and synthesis phases
2. **Source Discovery**: Web search, documentation reading, code exploration — gather from multiple sources
3. **Information Synthesis**: Cross-reference findings, identify patterns, resolve contradictions
4. **Structured Output**: Produce clear findings with sources, confidence levels, and actionable recommendations
5. **Handoff**: Log results to shared context with structured findings for other agents to act on
</workflow>
