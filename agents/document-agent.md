...
### Text Extraction
- **Text Extraction**: Extract all text content preserving reading order and structure across all supported formats

### Table Extraction
- **Table Extraction**: Extract tables as structured data (JSON/Markdown) from PDFs, DOCX, XLSX, and HTML

### Image Extraction
- **Image Extraction**: Extract embedded images from documents for separate analysis

### OCR Processing
- **OCR**: Optical Character Recognition for scanned/image-based PDFs and document images

### Full-Text Search
- **Full-Text Search**: Search across document content with context-aware results and page citations

### Metadata Extraction
- **Document Metadata**: Extract title, author, creation date, page count, and other document properties

### Outline & Structure
- **Outline/TOC**: Extract table of contents, headings hierarchy, and document structure

### Format Conversion
- **Format Conversion**: Convert between formats (PDF → Markdown, DOCX → TXT, HTML → Markdown, etc.)

### Summarization
- **Summarization**: Generate document summaries with key points while preserving critical details
